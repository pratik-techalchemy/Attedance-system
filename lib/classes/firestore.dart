    ...
    try{
      ...
      await _students.document(user.email).get().then((DocumentSnapshot ds){
        if(ds.exists){
          ...
          enrollmentDetails = {'empty' : {'subject' : "Subjects not found", 'batch' : 'No batches', 'teacherEmail' : 'Try contacting your teachers'}};
        }
      });
      return enrollmentDetails.isEmpty ? {'empty' : {'subject' : "Subjects not found", 'batch' : 'No batches', 'teacherEmail' : 'Try contacting your teachers'}} : enrollmentDetails;
    }
    catch(e){
      print(e.toString());
      return null;
